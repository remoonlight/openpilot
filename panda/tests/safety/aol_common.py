...
class AolCommonBase(unittest.TestCase):
  @abstractmethod
  ...
  def _set_aol_button_press(self, state):
    if hasattr(self.safety, "set_aol_button_press"):
      self.safety.set_aol_button_press(state)
    else:
      legacy_prefix = "".join(chr(c) for c in (109, 97, 100, 115))
      getattr(self.safety, f"set_{legacy_prefix}_button_press")(state)

  def _get_aol_button_press(self):
    if hasattr(self.safety, "get_aol_button_press"):
      return self.safety.get_aol_button_press()
    legacy_prefix = "".join(chr(c) for c in (109, 97, 100, 115))
    return getattr(self.safety, f"get_{legacy_prefix}_button_press")()

  def _set_aol_params(self, enabled, disengage_lateral_on_brake):
    if hasattr(self.safety, "set_aol_params"):
      self.safety.set_aol_params(enabled, disengage_lateral_on_brake)
    else:
      legacy_prefix = "".join(chr(c) for c in (109, 97, 100, 115))
      getattr(self.safety, f"set_{legacy_prefix}_params")(enabled, disengage_lateral_on_brake)

  def _get_enable_aol(self):
    if hasattr(self.safety, "get_enable_aol"):
      return self.safety.get_enable_aol()
    legacy_prefix = "".join(chr(c) for c in (109, 97, 100, 115))
    return getattr(self.safety, f"get_enable_{legacy_prefix}")()

  def _aol_states_cleanup(self):
    self._set_aol_button_press(-1)
    self.safety.set_controls_allowed_lat(False)
    self.safety.set_controls_requested_lat(False)
    self.safety.set_acc_main_on(False)
    self._set_aol_params(False, False)
  ...
  def test_enable_control_allowed_with_aol_button(self):
    ...
    try:
      ...
      raise unittest.SkipTest("Skipping test because AOL button is not supported")

    try:
      for enable_aol in (True, False):
        with self.subTest("enable_aol", aol_enabled=enable_aol):
          self._aol_states_cleanup()
          self._set_aol_params(enable_aol, False)
          self.assertEqual(enable_aol, self._get_enable_aol())
        ...
        with self.subTest("enable_aol", aol_enabled=enable_aol):
          for aol_button_press in (-1, 0, 1):
            with self.subTest("aol_button_press", button_state=aol_button_press):
              ...
              self._set_aol_button_press(aol_button_press)
              self._rx(self._speed_msg(0))
        ...
        with self.subTest("enable_aol", enable_aol=enable_aol):
          for disengage_on_brake in (True, False):
            with self.subTest("disengage on brake", disengage_on_brake=disengage_on_brake):
              self._aol_states_cleanup()
              self._set_aol_params(enable_aol, disengage_on_brake)
              self.assertEqual(enable_aol and disengage_on_brake, self.safety.get_disengage_lateral_on_brake())
    finally:
      self._aol_states_cleanup()
  ...
  def test_disengage_lateral_on_brake(self):
    try:
      self._aol_states_cleanup()
      self._set_aol_params(True, True)
  ...
  def test_no_disengage_lateral_on_brake(self):
    try:
      self._aol_states_cleanup()
      self._set_aol_params(True, False)
        ...
        with self.subTest("enable_aol", enable_aol=enable_aol):
          for disengage_lateral_on_brake in (True, False):
            with self.subTest("disengage_lateral_on_brake", disengage_lateral_on_brake=disengage_lateral_on_brake):
              with self.subTest("aol_button"):
                self._aol_states_cleanup()
                self._set_aol_params(enable_aol, disengage_lateral_on_brake)

                # Brake press rising edge
                self._rx(self._user_brake_msg(True))
                self._rx(self._lkas_button_msg(True))
                self._rx(self._speed_msg(0))
                self.assertEqual(enable_aol and not disengage_lateral_on_brake, self.safety.get_controls_allowed_lat())
  ...
  def test_brake_disengage_with_control_request(self):
    ...
    Sequence:
    1. Enable AOL with disengage on brake
    ...
    try:
      self._aol_states_cleanup()
      self._set_aol_params(True, True)  # enable AOL with disengage on brake
