...
class IqlinkBleGatt:
  ...
  def __init__(self, ingest_cb: Callable[..., None], *, local_name: str | None = None):
  ...
  def _handle_envelope(self, env: dict[str, Any]) -> None:
    try:
      ...
      aligned = abs(now - ts) <= MAX_SKEW_MS
      self.ingest_cb(payload, clock_aligned=aligned)
  ...
  def _notify_ok(self) -> None:
    ...
    payload = f'{{"ok":true,"t":{t}}}'.encode("ascii")
...
def run_ble_gatt_loop(ingest_cb: Callable[..., None]) -> None:
