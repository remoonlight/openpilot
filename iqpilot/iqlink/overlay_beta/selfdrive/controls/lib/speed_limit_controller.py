...
class IQSpeedLimitAssist:
  ...
  def update(self, enabled, v_ego, resolved_limit, resolved_source, slc_params, sm):
    ...
    if self._state == SpeedLimitAssistState.inactive:
      if has_limit:
        if self._needs_confirmation(resolved_limit, slc_params, resolved_source):
  ...
  def _needs_confirmation(self, new_limit, slc_params, source="None"):
    if not slc_params.get("speed_limit_controller", False):
      return False
    if source == "Iqlink":
      return False
