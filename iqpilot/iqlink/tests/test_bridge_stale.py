...
def test_ingest_does_not_write_navigation_destination():
  """Live guidance via iqNavState only; no stock NavigationDestination param."""
  p = FakeParams({})
  b = IqlinkBridge.__new__(IqlinkBridge)
  b.params = p
  b._lock = threading.Lock()
  ...
  payload = {
    ...
    "nGoPosTime": 1200,
    "szGoalName": "Test POI",
    "nGoalPosLat": 32030000,
    "nGoalPosLon": 118900000,
  }
  b.ingest(payload)
  assert b._latest is not None
  assert b._latest.get("destinationValid") is True
  assert p.get_bool("NavigationActive") is True
  assert p.get_bool("IqlinkExclusive") is True
  assert "NavigationDestination" not in p.values
...
def test_vision_stop_ignores_short_position_horizon():
  """pos.x[-1]<25 must not force visionStop (would block red prestart)."""
  b = IqlinkBridge.__new__(IqlinkBridge)
  mv = type("MV", (), {
    "action": type("A", (), {"shouldStop": False})(),
    "position": type("P", (), {"x": [0.0, 10.0]})(),
  })()
  b.sm = type("SM", (), {
    "update": lambda *a, **k: None,
    "alive": {"modelV2": True},
    "__getitem__": lambda self, k: mv,
  })()
  assert b._vision_stop() is False
  mv.action.shouldStop = True
  assert b._vision_stop() is True
