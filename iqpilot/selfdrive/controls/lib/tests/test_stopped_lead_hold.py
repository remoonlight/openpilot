...
def test_armed_holds_until_gap_opens():
  # Lead pulling away (vLead > LEAD_HOLD_V_ARM) must release so follow-lead can start.
  assert stopped_lead_hold(True, 0.0, STOPPING_SPEED, True, 3.6, 1.40, False) is False
  assert stopped_lead_hold(True, 0.0, STOPPING_SPEED, True, 4.9, 3.36, False) is False
  assert stopped_lead_hold(True, 0.0, STOPPING_SPEED, True, 5.6, 3.92, False) is False


def test_armed_holds_while_lead_still_stopped():
  assert stopped_lead_hold(True, 0.0, STOPPING_SPEED, True, 3.6, 0.0, False) is True
