...
IQLINK_EXIT_COMMIT_DISTANCE = 800.0  # aggressive CP-style early trigger
...
class NavExitLaneChangeController:
  ...
  def update(self, nav_state, carstate) -> None:
    ...
    # Aggressive LC window when fresh iqNavState + param; not a product "exclusive session".
    iqlink_lc = False
    try:
      iqlink_lc = bool(getattr(nav_state, "active", False)) and self._params.get_bool("IqlinkAggressiveLaneChange")
    except Exception:
      pass

    commit_m = IQLINK_EXIT_COMMIT_DISTANCE if iqlink_lc else NAV_EXIT_COMMIT_DISTANCE
    lc_desire = bool(getattr(nav_state, "shouldSendLaneChangeDesire", False))
    mtype = self._raw(getattr(nav_state, "nextManeuverType", _ManeuverType.none))
    is_exit = mtype == int(_ManeuverType.exit)
    if not (is_exit or (iqlink_lc and lc_desire)):
      return
    if not getattr(nav_state, "nextManeuverValid", False) and not lc_desire:
      return
    distance = float(getattr(nav_state, "nextManeuverDistance", 0.0))
    if not 0.0 < distance <= commit_m:
      return

    # Prefer explicit lane-change desire direction when present
    if lc_desire:
      direction = self._raw(getattr(nav_state, "laneChangeDesireDirection", _NavDirection.none))
      # laneChangeDesireDirection is IQTurnSignalDirection (turnLeft=1, turnRight=2)
      if direction == int(_NavDirection.left) or direction == 1:
        self.direction = log.LaneChangeDirection.left
      elif direction == int(_NavDirection.right) or direction == 2:
    ...
    # Still reject occupied adjacent lane; iqlink aggressive allows auto start when clear
    # even if BSM toggle is off (hard floor: blindspot blocks).
    if iqlink_lc:
      self.auto_allowed = not bool(blindspot)
    else:
