...
#endif
  // IQ.Pilot patch: iqpilot's common/util.h has no `check_system`. Inline an
  // equivalent — log on non-zero exit, otherwise behave like std::system.
  const int ret = std::system(command.c_str());
  if (ret != 0) {
    std::cerr << "open_external_url: command failed (exit " << ret << "): " << command << std::endl;
  }
}
