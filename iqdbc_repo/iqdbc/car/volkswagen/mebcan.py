...
def compute_meb_long_starting(long_control_state, v_ego, v_ego_starting, esp_hold_confirmation, stopping,
  ...
  # SET/RESUME @ hold must still release even while longState is stopping.
  if esp_hold_confirmation and cc_enabled:
    for be in button_events:
      if be.pressed and be.type in (ButtonType.setCruise, ButtonType.resumeCruise):
        return True
  # Follow-stop / lead hold: longControlState can flicker to pid while accel is
  # still negative. That used to set starting → ACC_HMS_RELEASE over esp_hold,
  # creep into the lead, then TSK=7 Cruise Faulted (2026-07-31 dump#6 / qlog 81-1).
  # Only auto-start when we are not still commanding a brake.
  if accel < 0:
    return False
  return (
...
def create_acc_accel_control(packer, bus, CP, acc_type, acc_enabled, upper_jerk, lower_jerk, upper_control_limit, lower_control_limit,
  ...
  # release-meb stop handshake: keep Anhalten=1 + real accel until ESP hold,
  # then ACCEL_INACTIVE. Early "near-zero = stopped" caused creep-phase
  # ACCEL_INACTIVE + Anhalten=0 → TSK=7 Cruise Faulted (qlog 079 dump).
