...
class MiciLabel(Widget):
  ...
  def _resolved_font(self) -> rl.Font:
    from openpilot.system.ui.lib.application import font_for_text
    return font_for_text(self.text, self.font_weight)
  ...
  def set_text(self, txt: str):
    if self.text != txt:
      ...
      self._scroll_state = ScrollState.STARTING
    self.text = txt
    text_size = measure_text_cached(self._resolved_font(), self.text, self.font_size, self.spacing)
    ...
    if self.wrap_text:
      self.wrapped_text = wrap_text(self._resolved_font(), self.text, self.font_size, int(self._rect.width))
...
class UnifiedLabel(Widget):
  ...
  def _update_text_cache(self, available_width: int):
    ...
    text = self.text
    from openpilot.system.ui.lib.application import font_for_text
    self._font = font_for_text(text, self._font_weight)
