...
def apply_iqlink_product_cruise_defaults(params: Params | None = None) -> None:
  """One-shot: overwrite stale cruise Params with iqlink product defaults.

  Fresh installs already get EnableSLPredReactToCurves=1 / LongIncrementsEnabled=1 /
  LongIncrementTapStep=10 from params_keys.h. Fielded devices often still hold older
  persisted 0/1 values — force once, then leave user toggles alone.
  """
  p = params if params is not None else Params()
