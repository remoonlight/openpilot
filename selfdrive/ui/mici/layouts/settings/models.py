...
class ModelsLayoutMici(NavScroller):
  def __init__(self):
    ...
    self._current = BigButton("current model", translate=True)
    self._current.set_click_callback(self._show_folders)

    self._cancel = BigButton("cancel download", translate=True)
    self._cancel.set_click_callback(self._cancel_model_request)
    self._cancel.set_visible(self._is_downloading)

    self._redownload = BigButton("redownload model", translate=True)
    self._redownload.set_click_callback(self._confirm_redownload_model)
    self._redownload.set_enabled(self._can_redownload)

    self._refresh = BigButton("refresh model list", translate=True)
    self._refresh.set_click_callback(lambda: ui_state.params.put("ModelManager_LastSyncTime", 0))

    self._supercombo = GreyBigButton("driving model", translate=True)
    self._supercombo.set_visible(False)
    self._vision = GreyBigButton("vision model", translate=True)
    self._vision.set_visible(False)
    self._policy = GreyBigButton("policy model", translate=True)
    self._policy.set_visible(False)

    self._clear = BigButton("clear model cache", translate=True)
    self._clear.set_click_callback(self._confirm_clear_cache)
    self._clear.set_enabled(lambda: ui_state.is_offroad())

    self._lagd = BigParamControl("live learning steer delay", "LagdToggle", translate=True)
    self._sw_delay = MappedParamToggle("software delay", "LagdToggleDelay", _DELAY_OPTIONS, _DELAY_VALUES, translate=True)
    self._sw_delay.set_visible(lambda: not self._lagd._checked)

    self._lane_turn = BigParamControl("use lane turn desires", "IQLaneTurnDesire", translate=True)
    self._lane_speed = MappedParamToggle("lane turn speed", "IQLaneTurnValue", _LANE_TURN_OPTIONS, _LANE_TURN_VALUES, translate=True)
