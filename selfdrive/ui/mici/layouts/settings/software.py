...
class CheckUpdateButton(BigButton):
  def __init__(self):
    ...
    super().__init__("check for update", "", self._txt_update_icon, translate=True)
...
class InstallUpdateButton(BigButton):
  def __init__(self):
    super().__init__("install update", "", gui_app.texture("icons_mici/settings/device/reboot.png", 64, 70), translate=True)
    self.set_press_effect_enabled(False)
...
class TargetBranchButton(BigButton):
  def __init__(self):
    super().__init__("target branch", ui_state.params.get("UpdaterTargetBranch") or "", translate=True)
    self.set_press_effect_enabled(False)
    self.set_click_callback(self._on_click)
...
class DisableUpdatesButton(BigButton):
  def __init__(self):
    super().__init__("disable\nupdates", tr("currently on"), translate=True)
    self.set_enabled(lambda: ui_state.is_offroad())
    self.set_press_effect_enabled(False)
    self._label.set_font_size(40)
    self._label.set_line_height(0.95)
