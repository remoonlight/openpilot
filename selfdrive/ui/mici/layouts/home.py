...
class DeviceStatus(Widget):
  ...
  def _get_version_text(self) -> str:
    ...
    if not description:
      ...
      if version and branch and commit:
        description = f"{version} / {branch} / {commit[:7]}"
...
class MiciHomeLayout(Widget):
  def __init__(self):
    ...
    self._bluetooth_txt = gui_app.texture("icons/iq/bluetooth.png", 40, 40, keep_aspect_ratio=True)
  ...
  def _get_version_text(self) -> tuple[str, str, str, str] | None:
    ...
    # Fallback: manager sets these at boot before updated fills UpdaterCurrentDescription
    version = ui_state.params.get("Version")
    branch = ui_state.params.get("GitBranch")
    commit = ui_state.params.get("GitCommit")
  ...
  def _render_bottom_status_bar(self):
    ...
    # iqlink BLE: hide when bridge off; green only when HMAC can push nav
    if iqlink_home_visible(ui_state.params):
      bt_y = self._rect.y + self.rect.height - self._bluetooth_txt.height / 2 - Y_CENTER
      bt_color = iqlink_status_color(ui_state.params)
      rl.draw_texture(self._bluetooth_txt, int(last_x), int(bt_y), bt_color)
      last_x += self._bluetooth_txt.width + ITEM_SPACING
