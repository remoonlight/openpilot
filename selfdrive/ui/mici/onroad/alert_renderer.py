...
from openpilot.selfdrive.ui.mici.mici_i18n import text_needs_noto_sc, script_font_runs
...
# CJK alert lines are short in char count but wide visually; keep them small.
ALERT_FONT_HAN = 36
...
class AlertRenderer(Widget):
  ...
  def _measure_mixed(self, text: str, font_size: int, latin_weight: FontWeight) -> float:
    width = 0.0
    for run, use_noto in script_font_runs(text):
      font = gui_app.font(FontWeight.NOTO_SANS_SC if use_noto else latin_weight)
      width += measure_text_cached(font, run, font_size).x
    return width

  def _draw_mixed_line(self, text: str, x: float, y: float, font_size: int, color: rl.Color,
                       latin_weight: FontWeight, align_right: bool, max_width: float) -> None:
    runs = script_font_runs(text)
    total_w = self._measure_mixed(text, font_size, latin_weight)
    cursor_x = x + max_width - total_w if align_right else x
    for run, use_noto in runs:
      font = gui_app.font(FontWeight.NOTO_SANS_SC if use_noto else latin_weight)
      rl.draw_text_ex(font, run, rl.Vector2(round(cursor_x), y), font_size, 0, color)
      cursor_x += measure_text_cached(font, run, font_size).x
  ...
  def _draw_text(self, alert: Alert, alert_layout: AlertLayout) -> None:
    ...
    if icon_side is not None:
      font_size -= 10 if not has_han else 4
    ...
    align_right = icon_side == 'left'
    if has_han:
      self._draw_mixed_line(
        alert_text1, text_rect1.x, text_rect1.y, font_size, color,
        FontWeight.DISPLAY, align_right, text_rect1.width,
      )
    else:
      ...
      self._alert_text1_label.set_alignment(rl.GuiTextAlignment.TEXT_ALIGN_RIGHT if align_right else rl.GuiTextAlignment.TEXT_ALIGN_LEFT)
    ...
    if can_draw_second_line and alert_text2:
      if has_han:
        last_line_h = text_rect1.y + font_size * FONT_SCALE
      else:
      ...
      if text_needs_noto_sc(alert_text2):
        self._draw_mixed_line(
          alert_text2, text_rect2.x, text_rect2.y, small_font_size, color,
          FontWeight.ROMAN, align_right, text_rect2.width,
        )
      else:
        ...
        self._alert_text2_label.set_alignment(rl.GuiTextAlignment.TEXT_ALIGN_RIGHT if align_right else rl.GuiTextAlignment.TEXT_ALIGN_LEFT)
