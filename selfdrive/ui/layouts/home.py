...
class HomeLayout(Widget):
  def __init__(self):
    ...
    # Launcher tiles (stock Mapbox Navigation tile removed — use iqlink)
    self._tiles = [
  ...
  def _get_version_texts(self) -> tuple[str, str]:
    # Called by HomeLayout._refresh for status-bar version chips.
    # Prefer UpdaterCurrentDescription; fall back to Version/Git* when updated hasn't run.
    description = self.params.get("UpdaterCurrentDescription")
    if not description:
      version = self.params.get("Version") or ""
      branch = self.params.get("GitBranch") or ""
      commit = (self.params.get("GitCommit") or "")[:7]
      if version:
        description = f"{version} / {branch} / {commit}"
