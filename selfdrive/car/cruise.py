...
V_CRUISE_MIN = 8
V_CRUISE_MAX = 200  # ~ 124 mph (safety ceiling; do not lower for product UX)
V_CRUISE_PRODUCT_MAX_KPH = 120  # ID.3 / iqlink product set-speed ceiling (avoid 119 from mph↔kph)
V_CRUISE_UNSET = 255
...
IQ_SET_SPEED_KPH_MAX = 120  # product max when treating fixed set as kph (metric)
...
class VCruiseHelperIQ:
  ...
  def get_iq_mode_initial_set_speed_kph(self, current_speed_kph: float, fallback_kph: float) -> float:
    ...
    if self.iq_set_speed_use_current:
      return float(np.clip(round(current_speed_kph, 1), self.v_cruise_min, V_CRUISE_PRODUCT_MAX_KPH))

    fixed_kph = float(self.iq_set_speed_mph) * CV.MPH_TO_KPH
    # 74 mph → ~119.1 kph; snap near-ceiling to exact product max so UI can show 120.
    if fixed_kph >= V_CRUISE_PRODUCT_MAX_KPH - 1.5:
      fixed_kph = float(V_CRUISE_PRODUCT_MAX_KPH)
    return float(np.clip(round(fixed_kph, 1), self.v_cruise_min, V_CRUISE_PRODUCT_MAX_KPH))
...
V_CRUISE_MIN = 8
V_CRUISE_MAX = 200  # ~ 124 mph (safety ceiling)
V_CRUISE_PRODUCT_MAX_KPH = 120  # product set-speed UX ceiling (reachable; not 119)
V_CRUISE_UNSET = 255
V_CRUISE_INITIAL = 40
...
class VCruiseHelper(VCruiseHelperIQ):
  ...
  def _update_v_cruise_non_pcm(self, CS, enabled, is_metric, allow_standby_adjustment=False):
    ...
    self.v_cruise_kph = np.clip(round(self.v_cruise_kph, 1), self.v_cruise_min, V_CRUISE_PRODUCT_MAX_KPH)
  ...
  def initialize_v_cruise(self, CS, experimental_mode: bool, iq_dynamic_mode: bool) -> None:
    ...
    if any(b.type in (ButtonType.accelCruise, ButtonType.resumeCruise) for b in CS.buttonEvents) and self.v_cruise_initialized:
      ...
      self.v_cruise_kph = int(round(np.clip(CS.vEgo * CV.MS_TO_KPH, initial, V_CRUISE_PRODUCT_MAX_KPH)))
