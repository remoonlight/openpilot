...
class DRIVER_MONITOR_SETTINGS:
  def __init__(self, device_type=None):
    ...
    self._ALWAYS_ON_ALERT_MIN_SPEED = 11  # always-on DM: suppress pre-alert while slow / not engaged
    ...
    # phone: uncalibrated absolute thresh; calibrated uses learned baseline * _PHONE_THRESH2
    _device = device_type if device_type is not None else HARDWARE.get_device_type()
    self._PHONE_THRESH = 0.75 if _device == 'mici' else 0.5
    ...
    self._PHONE_MAX_COUNT = int(360 / DT_DMON)
...
def face_orientation_from_net(angles_desc, pos_desc, rpy_calib):
  # parent-compatible import name; local path keeps pitch/yaw (+ roll passthrough)
...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False):
    ...
    self.phone_prob = 0.  # mirror of phone.prob for packet / callers
  ...
  def _get_distracted_types(self):
    ...
    if self.phone.prob_calibrated:
      phone_base = max(min(self.phone.prob_offseter.filtered_stat.M, self.settings._PHONE_MAX_OFFSET),
                       self.settings._PHONE_MIN_OFFSET)
      using_phone = self.phone.prob > phone_base * self.settings._PHONE_THRESH2
    else:
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, lowspeed, demo_mode=False, steering_angle_deg=0.):
    ...
    self.phone_prob = self.phone.prob
  ...
  def _update_events(self, driver_engaged, op_engaged, lowspeed, wrong_gear, car_speed=None):
    self.alert_level = AlertLevel.none
    self.driver_interacting = driver_engaged
    if car_speed is None:
      car_speed = 0. if lowspeed else self.settings._ALWAYS_ON_ALERT_MIN_SPEED
    ...
    _reaching_alert_2 = self.awareness - self.step_change <= self.threshold_alert_2
    ...
    if certainly_distracted or maybe_distracted:
      ...
      # always-on: hold at orange threshold while slow (parent helpers always_on_lowspeed_exemption)
      if not (lowspeed_exemption or always_on_exemption or (always_on_lowspeed_exemption and _reaching_alert_2)):
    ...
    if self.awareness <= 0.:
      ...
      elif self.awareness <= self.threshold_alert_1 and not always_on_lowspeed_exemption:
