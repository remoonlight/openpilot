...
# Only surface speed-camera HUD when the camera is near; far cameraValid packets
# were holding "Speed Camera" / "限速NNkm/h" for minutes (route 072).
SPEED_CAMERA_ALERT_MAX_DIST_M = 400.0
...
class SelfdriveD(GapButtonActions):
  ...
  def _refresh_cached_nav_events(self) -> None:
    ...
    if getattr(nav_state, 'active', False):
      # navTurnLeft/Right HUD ("转弯限速") suppressed; longitudinal still follows speedTarget.
    ...
    if getattr(nav_state, 'cameraValid', False):
      cam_type = getattr(nav_state, 'cameraType', None)
      cam_raw = int(getattr(cam_type, "raw", cam_type) or 0)
      # Do not surface red-light-as-camera ("红灯 Xm"); speed cams still alert as 限速NNkm/h.
      is_red = cam_type == custom.IQNavState.CameraType.redLight or cam_raw == int(custom.IQNavState.CameraType.redLight)
      is_alpr = cam_type == custom.IQNavState.CameraType.alpr or cam_raw == int(custom.IQNavState.CameraType.alpr)
      cam_dist = float(getattr(nav_state, 'cameraDistance', 0.0) or 0.0)
      # RF (BLE/WiFi) Flock/ALPR live hit: flockd/navd flag proximity with distance 0.
      flock_live = is_alpr and cam_dist <= 0.0
      if not is_red and (flock_live or 0.0 < cam_dist <= SPEED_CAMERA_ALERT_MAX_DIST_M):
  ...
  def update_events(self, CS):
    ...
    if not REPLAY:
      # PCM cruise on while OP off. Non-pcmCruise (OP long / MEB) keeps car cruiseState.enabled
      # while engaged — that is expected, not a mismatch (was flooding cruiseMismatch).
      cruise_mismatch = self.CP.pcmCruise and CS.cruiseState.enabled and not self.enabled
